...
export async function create2ContractPolygon(wallet: Signer, factoryOrContractJson: ContractJSON, args: any[] | undefined = [], overrideOptions = {}, contractPath: string = "", deployerAddress: string = SINGLETON_FACTORY_ADDRESS) {
  _init();
...
async function _deployer(wallet: Signer, initCode: string, salt: string, deployerAddress: string) {
  ...
  try {
    ...
    console.error("error in create2ContractPolygon._deployer");
    console.error(e);
    return ["0x0", "0"];
  }
}
