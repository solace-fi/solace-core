...
const DAI_ADDRESS                   = "0xE28bEf39f41c63B66cFD97BFfDB6Defc915B3C88";
...
const USDC_ADDRESS                  = "0x6D6DC3A8f02a1fEc0B9575e8dDE4135929Bd6e21";
...
const USDT_ADDRESS                  = "0x638b7BaA3D0C7d235fb904B01523883F980f24Ce";
...
const FRAX_ADDRESS                  = "0x86E5B6485e28E52a0dEEd28Cc10772FeB9c4C400";
...
const SLP_USDC_BOND_TELLER_ADDRESS  = "0x501acE55C9a613a15cDDbA158F67Ffca775842eB";
...
async function main() {
  ...
  await expectDeployed(SLP_USDC_ADDRESS);
...
async function deployEthTeller() {
  ...
  if(await isDeployed(ETH_BOND_TELLER_ADDRESS)) {
    ...
    let tx1 = await ethTeller.connect(deployer).initialize(NAME, signerAddress, solace.address, xslocker.address, UNDERWRITING_POOL_ADDRESS, DAO_ADDRESS, WETH_ADDRESS, false, bondDepo.address, networkSettings.overrides);
    await tx1.wait(networkSettings.confirmations);
    ...
    let tx3 = await bondDepo.connect(deployer).addTeller(ethTeller.address, networkSettings.overrides);
    await tx3.wait(networkSettings.confirmations);
    console.log('ETH teller - set fees');
    let tx4 = await ethTeller.connect(deployer).setFees(500, networkSettings.overrides);
    await tx4.wait(networkSettings.confirmations);
...
async function deploySlpUsdcTeller() {
  const NAME = "Solace SLP Bond";
  //const VESTING_TERM = 604800; // 7 days
  const VESTING_TERM = 600; // 10 minutes
  const HALF_LIFE = 600; // 10 minutes
  const ONE_CENT_IN_SLP = BN.from("28867513000");
  const ONE_TENTH_CENT_IN_SLP = BN.from("2886751300");

  const START_PRICE = ONE_CENT_IN_SLP.mul(8); // 8 cents
  const FLOOR_PRICE = ONE_CENT_IN_SLP.mul(3); // 3 cents
  const MAX_PAYOUT = BN.from("10000000000000000000000000") // 10 million SOLACE max single bond
  const CAPACITY = BN.from("100000000000000000000000000"); // 100 million SOLACE max over lifetime
  // every 50,000 SOLACE bonded raises the price one tenth of a cent
  const PRICE_ADJ_NUM = ONE_TENTH_CENT_IN_SLP; // tenth of a cent in SLP
  const PRICE_ADJ_DENOM = BN.from("50000000000000000000000"); // 50,000 SOLACE
  if(PRICE_ADJ_NUM.gt(MAX_UINT128) || PRICE_ADJ_DENOM.gt(MAX_UINT128)) throw `Uint128 too large: ${PRICE_ADJ_NUM.toString()} | ${PRICE_ADJ_DENOM.toString()} > ${MAX_UINT128.toString()}`;

  if(await isDeployed(SLP_USDC_BOND_TELLER_ADDRESS)) {
    slpUsdcTeller = (await ethers.getContractAt(artifacts.BondTellerERC20.abi, SLP_USDC_BOND_TELLER_ADDRESS)) as BondTellerErc20;
  } else {
    console.log("SLP Teller - deploy");
    var salt = "0x0000000000000000000000000000000000000000000000000000000005e96d75";
    slpUsdcTeller = await cloneTeller(daiTeller, NAME, SLP_USDC_ADDRESS, false, salt);
    console.log(`SLP Teller - deployed to ${slpUsdcTeller.address}`);
    console.log('SLP Teller - set terms');
    let tx2 = await slpUsdcTeller.connect(deployer).setTerms({startPrice: START_PRICE, minimumPrice: FLOOR_PRICE, maxPayout: MAX_PAYOUT, priceAdjNum: PRICE_ADJ_NUM, priceAdjDenom: PRICE_ADJ_DENOM, capacity: CAPACITY, capacityIsPayout: true, startTime: BOND_START_TIME, endTime: MAX_UINT40, globalVestingTerm: VESTING_TERM, halfLife: HALF_LIFE}, {...networkSettings.overrides, gasLimit: 300000});
    await tx2.wait(networkSettings.confirmations);
    console.log('SLP teller - add to bond depo');
    let tx3 = await bondDepo.connect(deployer).addTeller(slpUsdcTeller.address, networkSettings.overrides);
    await tx3.wait(networkSettings.confirmations);
    console.log('SLP Teller - done');
  }
}
...
async function logAddresses() {
  ...
  logContractAddress("SLP Bond Teller", slpUsdcTeller.address);
