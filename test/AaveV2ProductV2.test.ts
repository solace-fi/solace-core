...
import { PolicyManager, ProductFactory, BaseProductV2, Treasury, Weth9, ClaimsEscrow, Registry, Vault, RiskManager, MockAToken } from "../typechain";
...
if(process.env.FORK_NETWORK === "mainnet"){
  describe("AaveV2ProductV2", function () {
    ...
    let baseProduct: BaseProductV2;
    let product: BaseProductV2;
    let product2: BaseProductV2;
    let weth: Weth9;
    ...
    before(async function () {
      ...
      // deploy base product
      baseProduct = (await deployContract(deployer, artifacts.BaseProductV2)) as BaseProductV2;
    ...
    describe("deployment", function () {
      it("can deploy a product", async function () {
        let tx1 = await productFactory.createProduct(baseProduct.address, governor.address, policyManager.address, registry.address, AAVE_DATA_PROVIDER, minPeriod, maxPeriod, SUBMIT_CLAIM_TYPEHASH, DOMAIN_NAME, "1");
        let events1 = (await tx1.wait())?.events;
        if(events1 && events1.length > 0) {
          let event1 = events1[0];
          product = await ethers.getContractAt(artifacts.BaseProductV2.abi, event1?.args?.["deployment"]) as BaseProductV2;
      ...
      it("can deploy another", async function () {
        let tx2 = await productFactory.createProduct(baseProduct.address, governor.address, policyManager.address, registry.address, AAVE_DATA_PROVIDER, minPeriod, maxPeriod, SUBMIT_CLAIM_TYPEHASH, DOMAIN_NAME, "1");
        let events2 = (await tx2.wait())?.events;
        if(events2 && events2.length > 0) {
          let event2 = events2[0];
          product2 = await ethers.getContractAt(artifacts.BaseProductV2.abi, event2?.args?.["deployment"]) as BaseProductV2;
        } else throw "no deployment";

        expect(product.address != product2.address);
      });
      it("can deploy with create2", async function () {
        let product3: BaseProductV2;
        let product4: BaseProductV2;

        let predictedAddress1 = await productFactory.calculateMinimalProxyDeploymentAddress(baseProduct.address, toBytes32(0));
        let tx1 = await productFactory.create2Product(baseProduct.address, toBytes32(0), governor.address, policyManager.address, registry.address, AAVE_DATA_PROVIDER, minPeriod, maxPeriod, SUBMIT_CLAIM_TYPEHASH, DOMAIN_NAME, "1");
        let events1 = (await tx1.wait())?.events;
        if(events1 && events1.length > 0) {
          let event1 = events1[0];
          product3 = await ethers.getContractAt(artifacts.BaseProductV2.abi, event1?.args?.["deployment"]) as BaseProductV2;
        } else throw "no deployment";
        ...
        let tx2 = await productFactory.create2Product(baseProduct.address, toBytes32(1), governor.address, policyManager.address, registry.address, AAVE_DATA_PROVIDER, minPeriod, maxPeriod, SUBMIT_CLAIM_TYPEHASH, DOMAIN_NAME, "1");
        let events2 = (await tx2.wait())?.events;
        if(events2 && events2.length > 0) {
          let event2 = events2[0];
          product4 = await ethers.getContractAt(artifacts.BaseProductV2.abi, event2?.args?.["deployment"]) as BaseProductV2;
        } else throw "no deployment";
      ...
      it("cannot redeploy with same salt", async function () {
        await expect(productFactory.create2Product(baseProduct.address, toBytes32(1), governor.address, policyManager.address, registry.address, AAVE_DATA_PROVIDER, minPeriod, maxPeriod, SUBMIT_CLAIM_TYPEHASH, DOMAIN_NAME, "1")).to.be.revertedWith("Factory: failed deployment");
    ...
    describe("implementedFunctions", function () {
      ...
      it("can buy policy with malformed description", async function () {
        // our frontend knows what its doing, if the user malforms the description its on them
        let tx = await product.buyPolicy(REAL_USER1, coverAmount, blocks, "0x1234567890123456789012345678901234567890", { value: expectedPremium });
        expect(tx).to.emit(product, "PolicyCreated").withArgs(4);
        expect(await policyManager.totalSupply()).to.equal(4);
        expect(await policyManager.balanceOf(REAL_USER1)).to.equal(4);
      });
    })
