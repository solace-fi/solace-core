...
describe("PolicyManager", function () {
  ...
  describe("products", function () {
    ...
    it("can add products", async function () {
      ...
      await expect(tx1).to.emit(policyManager, "ProductAdded").withArgs(walletProduct1.address);
      ...
      await expect(tx2).to.emit(policyManager, "ProductAdded").withArgs(walletProduct2.address);
    })
    ...
    it("can remove products", async function () {
      ...
      await expect(tx1).to.emit(policyManager, "ProductRemoved").withArgs(walletProduct1.address);
