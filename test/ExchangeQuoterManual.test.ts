  ...
  describe("ExchangeQuoterManual", function () {
    ...
    describe("governance", function() {
      ...
      it("rejects setting new governance by non governor", async function() {
        await expect(quoter.connect(deployer).setGovernance(deployer.address)).to.be.revertedWith("!governance");
      });
      it("can set new governance", async function() {
        await quoter.connect(governor).setGovernance(deployer.address);
        expect(await quoter.governance()).to.equal(governor.address);
        expect(await quoter.newGovernance()).to.equal(deployer.address);
      });
      ...
      it("can transfer governance", async function() {
        ...
        expect(await quoter.newGovernance()).to.equal(ZERO_ADDRESS);
        await quoter.connect(deployer).setGovernance(governor.address);
