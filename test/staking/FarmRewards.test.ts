...
import { Solace, XSolacev1, FarmRewards, MockErc20Decimals, MockErc20Permit } from "./../../typechain";
...
describe("FarmRewards", function () {
  ...
  describe("deployment", function () {
    ...
    it("deploys successfully", async function () {
      ...
      await expectDeployed(farmRewards.address);
    ...
    it("returns correct values", async function () {
      ...
      expect(await farmRewards.VESTING_START()).eq(VESTING_START);
      expect(await farmRewards.VESTING_END()).eq(VESTING_END);
  ...
  describe("governance", function () {
    ...
    it("can set pending governance", async function () {
      ...
      await expect(tx).to.emit(farmRewards, "GovernancePending").withArgs(deployer.address);
      expect(await farmRewards.governance()).to.equal(governor.address);
  ...
  describe("receiver", function () {
    ...
    it("can be set", async function () {
      ...
      await expect(tx).to.emit(farmRewards, "ReceiverSet").withArgs(trader.address);
