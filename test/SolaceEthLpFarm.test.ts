...
describe('SolaceEthLpFarm', function () {
  ...
  describe('deposit and withdraw', function () {
    ...
    it('cannot deposit tokens from other pools', async function () {
      // high fee pool
      let tokenId1 = await mintLpToken(farmer1, weth, solaceToken, FeeAmount.HIGH, 1, getMinTick(TICK_SPACINGS[FeeAmount.HIGH]), getMaxTick(TICK_SPACINGS[FeeAmount.HIGH]));
      await lpToken.connect(farmer1).approve(farm.address, tokenId1);
      await expect(farm.connect(farmer1).deposit(tokenId1)).to.be.revertedWith('wrong pool');
      // mock token pool
      let tokenId2 = await mintLpToken(farmer1, mockToken1, solaceToken, FeeAmount.MEDIUM, 1, getMinTick(TICK_SPACINGS[FeeAmount.MEDIUM]), getMaxTick(TICK_SPACINGS[FeeAmount.MEDIUM]));
  ...
  describe('rewards', function () {
    ...
    it('accounts for token values', async function () {
      ...
      // range [-1, +2]
      tokenId1 = await mintLpToken(farmer1, weth, solaceToken, FeeAmount.MEDIUM, depositAmount1, tick.sub(1 * spacing), tick.add(2 * spacing));
      tokenValue1 = await lpTokenAppraisor.appraise(tokenId1);
      await lpToken.connect(farmer1).approve(farm.address, tokenId1);
      await farm.connect(farmer1).deposit(tokenId1);
      ...
      // range [0, +1]
      tokenId2 = await mintLpToken(farmer2, weth, solaceToken, FeeAmount.MEDIUM, depositAmount2, tick.add(0 * spacing), tick.add(1 * spacing));
      tokenValue2 = await lpTokenAppraisor.appraise(tokenId2);
      tokenValue12 = tokenValue1.add(tokenValue2);
      await lpToken.connect(farmer2).approve(farm.address, tokenId2);
      ...
      // range [+1, +2]
      tokenId3 = await mintLpToken(farmer2, weth, solaceToken, FeeAmount.MEDIUM, depositAmount3, tick.add(1 * spacing), tick.add(2 * spacing));
      tokenValue3 = await lpTokenAppraisor.appraise(tokenId3);
      tokenValue13 = tokenValue1.add(tokenValue3);
