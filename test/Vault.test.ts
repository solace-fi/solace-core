...
describe("Vault", function() {
  ...
  beforeEach(async () => {
    ...
    master = (await deployContract(owner, artifacts.Master, [newOwner.address, solace.address, solacePerBlock])) as Master;
    ...
    unaddedStrategy = (await deployContract(owner, artifacts.MockStrategy, [vault.address])) as MockStrategy;

    thirdStrategy = (await deployContract(owner, artifacts.MockStrategy, [vault.address])) as MockStrategy;
  ...
  describe("setGovernance", function() {
    it("should allow governance to set new governance address", async function() {
      ...
      await expect(tx)
        .to.emit(vault, "GovernanceTransferred")
  ...
  describe("updateStrategyDebtRatio", function() {
    ...
    it("should emit StrategyUpdateDebtRatio event with correct params", async function() {
      expect(await vault.connect(owner).updateStrategyDebtRatio(strategy.address, newDebtRatio))
        .to.emit(vault, "StrategyUpdateDebtRatio")
        .withArgs(strategy.address, newDebtRatio);
    });
  ...
  describe("updateStrategyMinDebtPerHarvest", function() {
    ...
    it("should emit StrategyUpdateDebtRatio event with correct params", async function() {
      expect(await vault.connect(owner).updateStrategyMinDebtPerHarvest(strategy.address, newMinDebtPerHarvest))
        .to.emit(vault, "StrategyUpdateMinDebtPerHarvest")
        .withArgs(strategy.address, newMinDebtPerHarvest);
    });
  });
  ...
  describe("updateStrategyMaxDebtPerHarvest", function() {
    ...
    it("should emit StrategyUpdateDebtRatio event with correct params", async function() {
      expect(await vault.connect(owner).updateStrategyMaxDebtPerHarvest(strategy.address, newMaxDebtPerHarvest))
        .to.emit(vault, "StrategyUpdateMaxDebtPerHarvest")
        .withArgs(strategy.address, newMaxDebtPerHarvest);
    });
  });
  ...
  describe("updateStrategyPerformanceFee", function() {
    ...
    it("should emit StrategyUpdateDebtRatio event with correct params", async function() {
      expect(await vault.connect(owner).updateStrategyPerformanceFee(strategy.address, newPerformanceFee))
        .to.emit(vault, "StrategyUpdatePerformanceFee")
        .withArgs(strategy.address, newPerformanceFee);
    });
  });
  ...
  describe("addStrategy", function() {
    ...
    it("should revert if invalid mix/max debt per harvest", async function() {
      await expect(vault.connect(owner).addStrategy(strategy.address, debtRatio, maxDebtPerHarvest, minDebtPerHarvest, performanceFee)).to.be.revertedWith(
        "minDebtPerHarvest exceeds maxDebtPerHarvest"
      );
    });
  });
  ...
  describe("withdrawal queue management", function() {
    ...
    describe("addStrategyToQueue", function() {
      ...
      it("should emit StrategyAddedToQueue event after function logic is successful", async function() {
        ...
        await expect(await vault.connect(owner).addStrategyToQueue(unaddedStrategy.address))
          .to.emit(vault, "StrategyAddedToQueue")
          .withArgs(unaddedStrategy.address);
      });
    });
    describe("removeStrategyFromQueue", function() {
      ...
      it("should emit StrategyRemovedFromQueue event after function logic is successful", async function() {
        await expect(await vault.connect(owner).removeStrategyFromQueue(strategy.address))
  ...
  describe("report", function() {
    ...
    it("should correctly account for losses reported by strategy", async function() {
      ...
      expect(await weth.balanceOf(strategy.address)).to.equal(maxDebt.gt(depositAmount / (MAX_BPS / debtRatio)) ? depositAmount / (MAX_BPS / debtRatio) : maxDebt);
  ...
  describe("deposit", function() {
    ...
    it("should emit Transfer event as CP tokens are minted", async function() {
      await expect(await vault.connect(depositor1).deposit({ value: testDepositAmount }))
        .to.emit(vault, "Transfer")
        .withArgs(ZERO_ADDRESS, depositor1.address, testDepositAmount);
    });
    it("should emit DepositMade event after function logic is successful", async function() {
      await expect(await vault.connect(depositor1).deposit({ value: testDepositAmount }))
        .to.emit(vault, "DepositMade")
        .withArgs(depositor1.address, testDepositAmount, testDepositAmount);
    });
    ...
    it("should hold if receive() sent by weth", async function() {
      let mockVault = (await deployContract(owner, artifacts.Vault, [owner.address, registry.address, depositor1.address])) as Vault;
      await depositor1.sendTransaction({
  ...
  describe("withdraw", function() {
    ...
    it("should withdraw 0/0", async function() {
      await expect(await vault.connect(depositor1).withdraw(0, 0))
        .to.emit(vault, "WithdrawalMade")
        .withArgs(depositor1.address, 0);
    });
    it("should withdraw with high degredation", async function() {
      ...
      await expect(await vault.connect(depositor1).withdraw(testDepositAmount, 0))
        .to.emit(vault, "WithdrawalMade")
        .withArgs(depositor1.address, testDepositAmount);
    });
    context("when there is enough WETH in the Vault", function() {
      ...
      it("should emit WithdrawalMade event after function logic is successful", async function() {
        ...
        await expect(await vaultDepositorSigner.withdraw(cpBalance, maxLoss))
          .to.emit(vault, "WithdrawalMade")
          .withArgs(depositor1.address, testDepositAmount);
      });
    });
    context("when there is not enough WETH in the Vault", function() {
      ...
      it("should withdraw from multiple strategies", async function() {
        ...
        await expect(await vault.connect(depositor1).withdraw(testDepositAmount, maxLoss))
          .to.emit(vault, "WithdrawalMade")
          .withArgs(depositor1.address, testDepositAmount);
      });
      ...
      it("should withdraw with acceptable loss", async function() {
        await strategy._takeFunds(1);
        await expect(await vault.connect(depositor1).withdraw(testDepositAmount, 10000))
          .to.emit(vault, "WithdrawalMade")
          .withArgs(depositor1.address, testDepositAmount.sub(1));
      });
    });
  });
