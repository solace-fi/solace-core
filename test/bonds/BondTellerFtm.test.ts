...
import { Solace, XsLocker, Weth9, Weth10, BondDepository, BondTellerFtm } from "./../../typechain";
...
let teller1: BondTellerFtm;
let teller2: BondTellerFtm;
...
describe("BondTellerFTM", function() {
  ...
  before(async function() {
    ...
    wftm9 = (await deployContract(deployer, artifacts.WETH)) as Weth9;
    wftm10 = (await deployContract(deployer, artifacts.WETH10)) as Weth10;
    await wftm9.connect(deployer).deposit({value: ONE_ETHER.mul(100)});
    await wftm10.connect(deployer).deposit({value: ONE_ETHER.mul(100)});
  ...
  describe("before initialization", function () {
    it("can deploy implementation", async function () {
      teller1 = (await deployContract(deployer, artifacts.BondTellerFTM)) as BondTellerFtm;
      await expectDeployed(teller1.address);
      await bondDepo.connect(governor).addTeller(teller1.address);
      teller2 = (await deployContract(deployer, artifacts.BondTellerFTM)) as BondTellerFtm;
    ...
    it("reverts if zero governor", async function () {
      await expect(teller1.initialize("Solace FTM Bond", ZERO_ADDRESS, solace.address, xslocker.address, underwritingPool.address, dao.address, wftm9.address, false, bondDepo.address)).to.be.revertedWith("zero address governance");
    });
    it("reverts if zero solace", async function () {
      await expect(teller1.initialize("Solace FTM Bond", governor.address, ZERO_ADDRESS, xslocker.address, underwritingPool.address, dao.address, wftm9.address, false, bondDepo.address)).to.be.revertedWith("zero address solace");
    });
    it("reverts if zero xslocker", async function () {
      await expect(teller1.initialize("Solace FTM Bond", governor.address, solace.address, ZERO_ADDRESS, underwritingPool.address, dao.address, wftm9.address, false, bondDepo.address)).to.be.revertedWith("zero address xslocker");
    });
    it("reverts if zero pool", async function () {
      await expect(teller1.initialize("Solace FTM Bond", governor.address, solace.address, xslocker.address, ZERO_ADDRESS, dao.address, wftm9.address, false, bondDepo.address)).to.be.revertedWith("zero address pool");
    });
    it("reverts if zero dao", async function () {
      await expect(teller1.initialize("Solace FTM Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, ZERO_ADDRESS, wftm9.address, false, bondDepo.address)).to.be.revertedWith("zero address dao");
    });
    it("reverts if zero principal", async function () {
      await expect(teller1.initialize("Solace FTM Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, ZERO_ADDRESS, false, bondDepo.address)).to.be.revertedWith("zero address principal");
    });
    it("reverts if zero bond depo", async function () {
      await expect(teller1.initialize("Solace FTM Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, wftm9.address, false, ZERO_ADDRESS)).to.be.revertedWith("zero address bond depo");
    });
  });

  describe("initialization", function () {
    it("inits", async function () {
      await teller1.initialize("Solace FTM Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, wftm9.address, false, bondDepo.address);
      await teller2.initialize("Solace FTM Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, wftm10.address, true, bondDepo.address);
    });
    it("inits with a name and symbol", async function () {
      expect(await teller1.name()).eq("Solace FTM Bond");
    ...
    it("starts with correct principal", async function () {
      expect(await teller1.principal()).eq(wftm9.address);
    });
  ...
  describe("pause/unpause", function () {
    ...
    it("cannot deposit while paused", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWftm(1, 1, depositor1.address, false)).to.be.revertedWith("cannot deposit while paused");
    ...
    it("cannot depositSigned while paused", async function () {
      ...
      let { v, r, s } = await getERC20PermitSignature(depositor1, teller2.address, wftm10, ONE_ETHER);
      await expect(teller2.connect(depositor1).depositWftmSigned(ONE_ETHER, ONE_ETHER, depositor1.address, false, deadline, v, r, s)).to.be.revertedWith("cannot deposit while paused");
  ...
  describe("before setTerms() called", function() {
    ...
    it("cannot deposit() or depositSigned()", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWftm(1, 1, depositor1.address, false)).to.be.revertedWith("not initialized");
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wftm10, ONE_ETHER);
      await expect(teller2.connect(depositor2).depositWftmSigned(ONE_ETHER, ONE_ETHER, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("not initialized");
    });
  });
  ...
  describe("ERC20 guards", function() {
    it("cannot deposit or depositSigned with insufficient balance", async function () {
      ...
      expect(await wftm9.balanceOf(depositor1.address)).to.equal(0);
      await expect(teller1.connect(depositor1).depositWftm(1, 1, depositor1.address, false)).to.be.revertedWith("ERC20: transfer amount exceeds balance");
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wftm10, 1);
      await expect(teller2.connect(depositor2).depositWftmSigned(1, 0, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("WETH: transfer amount exceeds balance");
    });
    it("cannot deposit without allowance", async function () {
      // Transfer 100 WFTM9 to depositor1
      await wftm9.connect(deployer).transfer(depositor1.address, 100);
      expect(await wftm9.balanceOf(depositor1.address)).to.equal(100)
      await expect(teller1.connect(deployer).depositWftm(100, 0, depositor1.address, false)).to.be.revertedWith("ERC20: transfer amount exceeds allowance");
      await wftm9.connect(depositor1).transfer(deployer.address, 100);
    });
    it("cannot permit a non erc20permit token", async function () {
      let { v, r, s } = await getERC20PermitSignature(depositor1, teller1.address, wftm9, 1, constants.MaxUint256, 1);
      await expect(teller1.connect(depositor1).depositWftmSigned(1, 1, depositor1.address, false, deadline, v, r, s)).to.be.revertedWith("principal does not support permit");
    });
  })

  describe("term parameter guards", function() {
    before(async function() {
      // Transfer 100K WFTM9 to depositor1
      await wftm9.connect(depositor1).approve(teller1.address, constants.MaxUint256);
      await wftm9.connect(deployer).transfer(depositor1.address, ONE_ETHER.mul(100));
      expect(await wftm9.balanceOf(depositor1.address)).to.equal(ONE_ETHER.mul(100));

      // Transfer 100K WFTM10 to depositor2
      await wftm10.connect(deployer).transfer(depositor2.address, ONE_ETHER.mul(100));
      expect(await wftm10.balanceOf(depositor2.address)).to.equal(ONE_ETHER.mul(100));
    })

    it("cannot deposit or depositSigned with a zero address depositor", async function() {
      await expect(teller1.connect(depositor1).depositWftm(1, 1, ZERO_ADDRESS, false)).to.be.revertedWith("invalid address");
      ...
      // let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wftm10, ONE_ETHER);
      // await expect(teller2.connect(depositor2).depositWftmSigned(ONE_ETHER, ONE_ETHER, ZERO_ADDRESS, false, deadline, v, r, s)).to.be.revertedWith("invalid address");
    })
    it("cannot deposit or depositSigned before startTime", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWftm(1, 1, depositor1.address, false)).to.be.revertedWith("bond not yet started");

      await teller2.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wftm10, ONE_ETHER);
      await expect(teller2.connect(depositor2).depositWftmSigned(ONE_ETHER, ONE_ETHER, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("bond not yet started");
    });
    it("cannot deposit or depositSigned after endTime", async function () {
      ...
      await teller1.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      await expect(teller1.connect(depositor1).depositWftm(1, 1, depositor1.address, false)).to.be.revertedWith("bond concluded");

      await teller2.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wftm10, ONE_ETHER);
      await expect(teller2.connect(depositor2).depositWftmSigned(ONE_ETHER, ONE_ETHER, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("bond concluded");
    });
    it("cannot deposit or depositSigned if bondPrice decayed to 0", async function() {
      ...
      await expect(teller1.connect(depositor1).depositWftm(1, 1, depositor1.address, false)).to.be.revertedWith("invalid price");
      ...
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wftm10, ONE_ETHER);
      await expect(teller2.connect(depositor2).depositWftmSigned(ONE_ETHER, ONE_ETHER, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("invalid price");
    })
    it("Given capacityIsPayout = false, deposit or depositSigned will revert if `principal paid > capacity`", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWftm(ONE_ETHER.add(1), ONE_ETHER.mul(2), depositor1.address, false)).to.be.revertedWith("bond at capacity");
      ...
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wftm10, ONE_ETHER.add(1));
      await expect(teller2.connect(depositor2).depositWftmSigned(ONE_ETHER.add(1), ONE_ETHER, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("bond at capacity");
    });
    it("Given capacityIsPayout = true, deposit or depositSigned will revert if `payout > capacity`", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWftm(ONE_ETHER.add(2), 0, depositor1.address, false)).to.be.revertedWith("bond at capacity");
      ...
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wftm10, ONE_ETHER.add(2));
      await expect(teller2.connect(depositor2).depositWftmSigned(ONE_ETHER.add(2), 0, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("bond at capacity");
    });
    it("calculateAmountIn, calculateAmountOut, deposit and depositSigned will respect maxPayout", async function() {
      ...
      await expect(teller1.connect(depositor1).depositWftm(ONE_ETHER.add(2), 0, depositor1.address, false)).to.be.revertedWith("bond too large");
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wftm10, ONE_ETHER.add(2));
      await expect(teller2.connect(depositor2).depositWftmSigned(ONE_ETHER.add(2), 0, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("bond too large");
    })
    it("slippage protection - deposit() and depositSigned() respect minAmountOut", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWftm(amountIn1.sub(1), ONE_ETHER, depositor1.address, false)).to.be.revertedWith("slippage protection");
      // Block-scope here to avoid v, r, s namespace conflict
      {
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wftm10, amountIn1.sub(1));
      await expect(teller2.connect(depositor2).depositWftmSigned(amountIn1.sub(1), ONE_ETHER, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("slippage protection");
      }

      const amountIn2 = await teller1.calculateAmountIn(ONE_ETHER, true);
      await expect(teller1.connect(depositor1).depositWftm(amountIn2.sub(1), ONE_ETHER, depositor1.address, true)).to.be.revertedWith("slippage protection");
      {
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wftm10, amountIn2.sub(1));
      await expect(teller2.connect(depositor2).depositWftmSigned(amountIn2.sub(1), ONE_ETHER, depositor2.address, true, deadline, v, r, s)).to.be.revertedWith("slippage protection");
      }
    });
    it("cannot deposit if bondDepo is not solace minter", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWftm(2, 0, depositor1.address, false)).to.be.revertedWith("!minter");
      await solace.connect(governor).addMinter(bondDepo.address);
    });
    it("cannot deposit if teller not registered", async function () {
      await bondDepo.connect(governor).removeTeller(teller1.address);
      await expect(teller1.connect(depositor1).depositWftm(2, 0, depositor1.address, false)).to.be.revertedWith("!teller");
      await bondDepo.connect(governor).addTeller(teller1.address);
    });
  })

  describe("depositFtm cases", function () {
    it("test deposit 1 - deposit 3 FTM, starting SOLACE price of 2 FTM", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      await teller1.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      ...
      // Tx to purchase bond
      let tx1 = await teller1.connect(depositor1).depositFtm(ONE_ETHER, depositor1.address, false, {value: ONE_ETHER.mul(3)});
      ...
      expect(change_in_balances.userFtm).eq(ONE_ETHER.mul(-3).sub(gasCost));
      expect(change_in_balances.userWftm9).eq(0);
      expect(change_in_balances.userWftm10).eq(0);
      expect(change_in_balances.daoFtm).eq(ONE_ETHER.mul(3).mul(protocolFee).div(MAX_BPS));
      expect(change_in_balances.daoWftm9).eq(0);
      expect(change_in_balances.daoWftm10).eq(0);
      expect(change_in_balances.poolFtm).eq(ONE_ETHER.mul(3).mul(MAX_BPS-protocolFee).div(MAX_BPS))
      expect(change_in_balances.poolWftm9).eq(0);
      expect(change_in_balances.poolWftm10).eq(0);
    ...
    it("test deposit 2 - capacityIsPayout = true && stake = true, deposit 3 FTM, starting SOLACE price of 2 FTM", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      MODIFIED_BOND_TERMS.capacityIsPayout = true;
      ...
      // Deposit
      let tx1 = await teller1.connect(depositor1).depositFtm(ONE_ETHER, depositor1.address, true, {value: ONE_ETHER.mul(3)});
      let receipt = await tx1.wait();
      let gasCost = receipt.gasUsed.mul(receipt.effectiveGasPrice);
    ...
    it("test deposit 3 - deposit 3 FTM, set startPrice = 1 but minimumPrice = 2", async function() {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
  ...
  describe("claimPayout after depositFtm cases", function() {
    ...
    it("t = 0, expect claimPayout will work but there will be miniscule payout", async function() {
      ...
      expect(change_in_balances.userWftm9).to.eq(0)
      expect(change_in_balances.userWftm10).to.eq(0)
      expect(change_in_balances.daoWftm9).to.eq(0)
      expect(change_in_balances.daoWftm10).to.eq(0)
      expect(change_in_balances.poolWftm9).to.eq(0)
      expect(change_in_balances.poolWftm10).to.eq(0)
      ...
      expect(change_in_balances_depositor2.userWftm9).to.eq(0)
      expect(change_in_balances_depositor2.userWftm10).to.eq(0)
  ...
  describe("depositWftm cases", function () {
    before("redeploy", async function () {
      ...
      await solace.connect(governor).addMinter(bondDepo.address);
      teller1 = (await deployContract(deployer, artifacts.BondTellerFTM)) as BondTellerFtm;
      await teller1.connect(governor).initialize("Solace WFTM9 Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, wftm9.address, false, bondDepo.address);
      await bondDepo.connect(governor).addTeller(teller1.address);
      teller2 = (await deployContract(deployer, artifacts.BondTellerFTM)) as BondTellerFtm;
      await teller2.connect(governor).initialize("Solace WFTM10 Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, wftm10.address, true, bondDepo.address);
      await bondDepo.connect(governor).addTeller(teller2.address);
      await wftm9.connect(depositor1).approve(teller1.address, constants.MaxUint256);
    });
    it("test deposit 1 - deposit 3 WFTM9, starting SOLACE price of 2 WFTM9", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      await teller1.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      ...
      // Tx to purchase bond
      let tx1 = await teller1.connect(depositor1).depositWftm(ONE_ETHER.mul(3), ONE_ETHER, depositor1.address, false);
      let receipt = await tx1.wait();
      let gasCost = receipt.gasUsed.mul(receipt.effectiveGasPrice);
      ...
      expect(change_in_balances.vestingSolace).eq(bondInfo.payoutAmount);
      expect(change_in_balances.stakingSolace).eq(0);
      expect(change_in_balances.userFtm).eq(gasCost.mul(-1));
      expect(change_in_balances.userWftm9).eq(ONE_ETHER.mul(-3));
      expect(change_in_balances.userWftm10).eq(0);
      expect(change_in_balances.daoFtm).eq(0);
      expect(change_in_balances.daoWftm9).eq(ONE_ETHER.mul(3).mul(protocolFee).div(MAX_BPS));
      expect(change_in_balances.daoWftm10).eq(0);
      expect(change_in_balances.poolFtm).eq(0);
      expect(change_in_balances.poolWftm9).eq(ONE_ETHER.mul(3).mul(MAX_BPS-protocolFee).div(MAX_BPS))
      expect(change_in_balances.poolWftm10).eq(0);
      expect(change_in_balances.userBonds).eq(1);
      expect(change_in_balances.totalBonds).eq(1);
      expect(change_in_balances.userLocks).eq(0);
    ...
    it("test deposit 2 - capacityIsPayout = true && stake = true, deposit 3 WFTM9, starting SOLACE price of 2 WFTM9", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      MODIFIED_BOND_TERMS.capacityIsPayout = true;
      ...
      // Deposit
      let tx1 = await teller1.connect(depositor1).depositWftm(ONE_ETHER.mul(3), ONE_ETHER, depositor1.address, true);
      let receipt = await tx1.wait();
      let gasCost = receipt.gasUsed.mul(receipt.effectiveGasPrice);
    ...
    it("test deposit 3 - deposit 3 WFTM9, set startPrice = 1 but minimumPrice = 2", async function() {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.minimumPrice = ONE_ETHER.mul(2)
      await teller1.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      let protocolFee = PROTOCOL_FEE;
  ...
  describe("claimPayout after depositWftm cases", function() {
    it("cannot claimPayout for a non-existent bondID", async function () {
      await expect(teller1.connect(depositor1).claimPayout(999)).to.be.revertedWith("query for nonexistent token");
    });
    it("cannot claimPayout for a bondID that you are not that owner of, or are approved to transfer", async function () {
  ...
  describe("depositWftmSigned cases", function () {
    before("redeploy", async function () {
      solace = (await deployContract(deployer, artifacts.SOLACE, [governor.address])) as Solace;
      xslocker = (await deployContract(deployer, artifacts.xsLocker, [governor.address, solace.address])) as XsLocker;
      bondDepo = (await deployContract(deployer, artifacts.BondDepository, [governor.address, solace.address])) as BondDepository;
      ...
      await teller2.connect(governor).initialize("Solace WFTM10 Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, wftm10.address, true, bondDepo.address);
      await bondDepo.connect(governor).addTeller(teller2.address);
      await wftm9.connect(depositor1).approve(teller2.address, constants.MaxUint256);
      await wftm10.connect(depositor1).approve(teller2.address, constants.MaxUint256);
      await wftm10.connect(depositor1).deposit({value: ONE_ETHER.mul(100)});
    });
    it("test deposit 1 - deposit 3 WFTM10, starting SOLACE price of 2 WFTM10", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      await teller2.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      ...
      // Tx to purchase bond
      let { v, r, s } = await getERC20PermitSignature(depositor1, teller2.address, wftm10, ONE_ETHER.mul(3));
      let tx1 = await teller2.connect(depositor1).depositWftmSigned(ONE_ETHER.mul(3), ONE_ETHER, depositor1.address, false, deadline, v, r, s);
      let receipt = await tx1.wait();
      let gasCost = receipt.gasUsed.mul(receipt.effectiveGasPrice);
      ...
      expect(change_in_balances.userFtm).eq(gasCost.mul(-1));
      expect(change_in_balances.userWftm9).eq(0);
      expect(change_in_balances.userWftm10).eq(ONE_ETHER.mul(-3));
      expect(change_in_balances.daoFtm).eq(0);
      expect(change_in_balances.daoWftm9).eq(0);
      expect(change_in_balances.daoWftm10).eq(ONE_ETHER.mul(3).mul(protocolFee).div(MAX_BPS));
      expect(change_in_balances.poolFtm).eq(0);
      expect(change_in_balances.poolWftm9).eq(0);
      expect(change_in_balances.poolWftm10).eq(ONE_ETHER.mul(3).mul(MAX_BPS-protocolFee).div(MAX_BPS))
      expect(change_in_balances.userBonds).eq(1);
      expect(change_in_balances.totalBonds).eq(1);
      expect(change_in_balances.userLocks).eq(0);
      expect(change_in_balances.totalLocks).eq(0);
    ...
    it("test deposit 2 - capacityIsPayout = true && stake = true, deposit 3 WFTM10, starting SOLACE price of 2 WFTM10", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      MODIFIED_BOND_TERMS.capacityIsPayout = true;
      ...
      // Deposit
      let { v, r, s } = await getERC20PermitSignature(depositor1, teller2.address, wftm10, ONE_ETHER.mul(3));
      let tx1 = await teller2.connect(depositor1).depositWftmSigned(ONE_ETHER.mul(3), ONE_ETHER, depositor1.address, true, deadline, v, r, s);
      let receipt = await tx1.wait();
      let gasCost = receipt.gasUsed.mul(receipt.effectiveGasPrice);
    ...
    it("test deposit 3 - deposit 3 WFTM10, set startPrice = 1 but minimumPrice = 2", async function() {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.minimumPrice = ONE_ETHER.mul(2)
      await teller2.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      let protocolFee = PROTOCOL_FEE;
  ...
  describe("claimPayout after depositWftmSigned cases", function() {
    it("cannot claimPayout for a non-existent bondID", async function () {
  ...
  describe("set addresses", function () {
    ...
    it("non governance cannot set addresses", async function () {
      await expect(teller1.connect(depositor1).setAddresses(solace.address, xslocker.address, underwritingPool.address, dao.address, wftm9.address, false, bondDepo2.address)).to.be.revertedWith("!governance");
    });
    it("validates input", async function () {
      await expect(teller1.connect(governor).setAddresses(ZERO_ADDRESS, xslocker.address, underwritingPool.address, dao.address, wftm9.address, false, bondDepo2.address)).to.be.revertedWith("zero address solace");
      await expect(teller1.connect(governor).setAddresses(solace.address, ZERO_ADDRESS, underwritingPool.address, dao.address, wftm9.address, false, bondDepo2.address)).to.be.revertedWith("zero address xslocker");
      await expect(teller1.connect(governor).setAddresses(solace.address, xslocker.address, ZERO_ADDRESS, dao.address, wftm9.address, false, bondDepo2.address)).to.be.revertedWith("zero address pool");
      await expect(teller1.connect(governor).setAddresses(solace.address, xslocker.address, underwritingPool.address, ZERO_ADDRESS, wftm9.address, false, bondDepo2.address)).to.be.revertedWith("zero address dao");
      ...
      await expect(teller1.connect(governor).setAddresses(solace.address, xslocker.address, underwritingPool.address, dao.address, wftm9.address, false, ZERO_ADDRESS)).to.be.revertedWith("zero address bond depo");
    })
    it("governance can set addresses", async function () {
      let tx = await teller1.connect(governor).setAddresses(solace2.address, xslocker2.address, underwritingPool2.address, dao2.address, wftm10.address, true, bondDepo2.address);
      ...
      expect(await teller1.principal()).eq(wftm10.address);
    ...
    it("uses new addresses", async function () {
      await wftm10.connect(depositor1).approve(teller1.address, constants.MaxUint256);
      ...
      expect(bal12.userWftm9).eq(0);
      expect(bal12.userWftm10).eq(ONE_ETHER.mul(-3));
      expect(bal12.daoWftm9).eq(0);
      expect(await wftm10.balanceOf(dao2.address)).eq(ONE_ETHER.mul(3).mul(PROTOCOL_FEE).div(MAX_BPS));
      expect(bal12.poolWftm9).eq(0);
      expect(await wftm10.balanceOf(underwritingPool2.address)).eq(ONE_ETHER.mul(3).mul(MAX_BPS-PROTOCOL_FEE).div(MAX_BPS));
  ...
  async function getBalances(teller: Contract, user: Wallet): Promise<Balances> {
    return {
      ...
      userFtm: await provider.getBalance(user.address),
      userWftm9: await wftm9.balanceOf(user.address),
      userWftm10: await wftm10.balanceOf(user.address),
      daoFtm: await provider.getBalance(dao.address),
      daoWftm9: await wftm9.balanceOf(dao.address),
      daoWftm10: await wftm10.balanceOf(dao.address),
      poolFtm: await provider.getBalance(underwritingPool.address),
      poolWftm9: await wftm9.balanceOf(underwritingPool.address),
      poolWftm10: await wftm10.balanceOf(underwritingPool.address),
  ...
  function getBalancesDiff(balances1: Balances, balances2: Balances): Balances {
    return {
      ...
      userFtm: balances1.userFtm.sub(balances2.userFtm),
      userWftm9: balances1.userWftm9.sub(balances2.userWftm9),
      userWftm10: balances1.userWftm10.sub(balances2.userWftm10),
      daoFtm: balances1.daoFtm.sub(balances2.daoFtm),
      daoWftm9: balances1.daoWftm9.sub(balances2.daoWftm9),
      daoWftm10: balances1.daoWftm10.sub(balances2.daoWftm10),
      poolFtm: balances1.poolFtm.sub(balances2.poolFtm),
      poolWftm9: balances1.poolWftm9.sub(balances2.poolWftm9),
      poolWftm10: balances1.poolWftm10.sub(balances2.poolWftm10),
