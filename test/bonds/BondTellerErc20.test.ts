...
import { Solace, XsLocker, MockErc20, MockErc20Permit, BondDepository, BondTellerErc20 } from "./../../typechain";
...
describe("BondTellerERC20", function() {
  ...
  describe("before initialization", function () {
    ...
    it("reverts if zero governor", async function () {
      await expect(teller1.initialize("Solace DAI Bond", ZERO_ADDRESS, solace.address, xslocker.address, underwritingPool.address, dao.address, dai.address, false, bondDepo.address)).to.be.revertedWith("zero address governance");
    });
    it("reverts if zero solace", async function () {
      await expect(teller1.initialize("Solace DAI Bond", governor.address, ZERO_ADDRESS, xslocker.address, underwritingPool.address, dao.address, dai.address, false, bondDepo.address)).to.be.revertedWith("zero address solace");
    });
    it("reverts if zero xslocker", async function () {
      await expect(teller1.initialize("Solace DAI Bond", governor.address, solace.address, ZERO_ADDRESS, underwritingPool.address, dao.address, dai.address, false, bondDepo.address)).to.be.revertedWith("zero address xslocker");
    });
    it("reverts if zero pool", async function () {
      await expect(teller1.initialize("Solace DAI Bond", governor.address, solace.address, xslocker.address, ZERO_ADDRESS, dao.address, dai.address, false, bondDepo.address)).to.be.revertedWith("zero address pool");
    });
    it("reverts if zero dao", async function () {
      await expect(teller1.initialize("Solace DAI Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, ZERO_ADDRESS, dai.address, false, bondDepo.address)).to.be.revertedWith("zero address dao");
    });
    it("reverts if zero principal", async function () {
      await expect(teller1.initialize("Solace DAI Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, ZERO_ADDRESS, false, bondDepo.address)).to.be.revertedWith("zero address principal");
    });
    it("reverts if zero bond depo", async function () {
      await expect(teller1.initialize("Solace DAI Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, dai.address, false, ZERO_ADDRESS)).to.be.revertedWith("zero address bond depo");
    });
  });

  describe("initialization", function () {
    it("inits", async function () {
      await teller1.initialize("Solace DAI Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, dai.address, false, bondDepo.address);
    });
    ...
    it("starts with correct principal", async function () {
      ...
      expect(await teller1.isPermittable()).eq(false);
    });
  ...
  describe("clone", function () {
    ...
    it("starts with correct xslocker", async function () {
      expect(await teller2.xsLocker()).eq(xslocker.address);
    });
    it("starts with correct pool", async function () {
    ...
    it("starts with correct principal", async function () {
      expect(await teller2.principal()).eq(usdc.address);
      expect(await teller2.isPermittable()).eq(true);
    });
  });
  ...
  describe("pause/unpause", function () {
    ...
    it("cannot depositSigned while paused", async function () {
      ...
      let { v, r, s } = await getERC20PermitSignature(depositor1, teller2.address, usdc, ONE_ETHER);
  ...
  describe("before setTerms() called", function() {
    ...
    it("cannot deposit() or depositSigned()", async function () {
      ...
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, usdc, ONE_ETHER);
  ...
  describe("ERC20 guards", function() {
    it("cannot deposit or depositSigned with insufficient balance", async function () {
      ...
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, usdc, 1);
    ...
    it("cannot permit a non erc20permit token", async function () {
      ...
      await expect(teller1.connect(depositor1).depositSigned(1, 1, depositor1.address, false, deadline, v, r, s)).to.be.revertedWith("principal does not support permit");
    });
    it("cannot permit a non erc20permit token even if wrongly initialized", async function () {
      teller3 = await cloneTeller(teller1, "Solace USDC Bond", dai.address, true);
      await bondDepo.connect(governor).addTeller(teller2.address);
      let { v, r, s } = await getERC20PermitSignature(depositor1, teller3.address, dai, 1, constants.MaxUint256, 1);
      await expect(teller3.connect(depositor1).depositSigned(1, 1, depositor1.address, false, deadline, v, r, s)).to.be.revertedWith("Transaction reverted: function selector was not recognized and there's no fallback function");
    });
  })
  ...
  describe("term parameter guards", function() {
    before(async function() {
      ...
      // Transfer 100K USDC to depositor2
      await usdc.connect(deployer).transfer(depositor2.address, ONE_HUNDRED_THOUSAND_ETHER);
      expect(await usdc.balanceOf(depositor2.address)).to.equal(ONE_HUNDRED_THOUSAND_ETHER);
    })
    ...
    it("Given capacityIsPayout = false, deposit or depositSigned will revert if `principal paid > capacity`", async function () {
      ...
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, usdc, ONE_ETHER.add(1));
    ...
    it("Given capacityIsPayout = true, deposit or depositSigned will revert if `payout > capacity`", async function () {
      ...
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, usdc, ONE_ETHER.add(2));
    ...
    it("slippage protection - deposit() and depositSigned() respect minAmountOut", async function () {
      ...
      await expect(teller1.connect(depositor1).deposit(amountIn1.sub(1), ONE_ETHER, depositor1.address, false)).to.be.revertedWith("slippage protection");
      // Block-scope here to avoid v, r, s namespace conflict
      {
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, usdc, amountIn1.sub(1));
      await expect(teller2.connect(depositor2).depositSigned(amountIn1.sub(1), ONE_ETHER, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("slippage protection");
      }

      const amountIn2 = await teller1.calculateAmountIn(ONE_ETHER, true);
      await expect(teller1.connect(depositor1).deposit(amountIn2.sub(1), ONE_ETHER, depositor1.address, true)).to.be.revertedWith("slippage protection");
      {
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, usdc, amountIn2.sub(1));
      await expect(teller2.connect(depositor2).depositSigned(amountIn2.sub(1), ONE_ETHER, depositor2.address, true, deadline, v, r, s)).to.be.revertedWith("slippage protection");
      }
    });
    it("cannot deposit if bondDepo is not solace minter", async function () {
      ...
      await expect(teller1.connect(depositor1).deposit(2, 0, depositor1.address, false)).to.be.revertedWith("!minter");
      await solace.connect(governor).addMinter(bondDepo.address);
    });
    it("cannot deposit if teller not registered", async function () {
      await bondDepo.connect(governor).removeTeller(teller1.address);
      await expect(teller1.connect(depositor1).deposit(2, 0, depositor1.address, false)).to.be.revertedWith("!teller");
      await bondDepo.connect(governor).addTeller(teller1.address);
    });
  })
  ...
  describe("deposit cases", function() {
    it("test deposit 1 - deposit 3 DAI, starting SOLACE price of 2 DAI", async function () {
      ...
      expect(change_in_balances.userUsdc).eq(0);
      expect(change_in_balances.daoDai).eq(ONE_ETHER.mul(3).mul(protocolFee).div(MAX_BPS));
      expect(change_in_balances.daoUsdc).eq(0);
      expect(change_in_balances.poolDai).eq(ONE_ETHER.mul(3).mul(MAX_BPS-protocolFee).div(MAX_BPS))
      expect(change_in_balances.poolUsdc).eq(0);
  ...
  describe("claimPayout after deposit cases", function() {
    ...
    it("t = 0, expect claimPayout will work but there will be miniscule payout", async function() {
      ...
      expect(change_in_balances.userUsdc).to.eq(0)
      expect(change_in_balances.daoDai).to.eq(0)
      expect(change_in_balances.daoUsdc).to.eq(0)
      expect(change_in_balances.poolDai).to.eq(0)
      expect(change_in_balances.poolUsdc).to.eq(0)
      ...
      expect(change_in_balances_depositor2.userUsdc).to.eq(0)
  ...
  describe("depositSigned cases", function() {
    before("redeploy", async function () {
      ...
      await solace.connect(governor).addMinter(bondDepo.address);
      teller1 = (await deployContract(deployer, artifacts.BondTellerERC20)) as BondTellerErc20;
      await teller1.connect(governor).initialize("Solace DAI Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, usdc.address, false, bondDepo.address);
      await bondDepo.connect(governor).addTeller(teller1.address);
      teller2 = await cloneTeller(teller1, "Solace USDC Bond", usdc.address, true);
      await bondDepo.connect(governor).addTeller(teller2.address);
    });
    it("test deposit 1 - deposit 3 USDC, starting SOLACE price of 2 USDC", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      await teller2.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      ...
      await usdc.connect(deployer).transfer(depositor1.address, ONE_ETHER.mul(100));
      ...
      // Tx to purchase bond
      let { v, r, s } = await getERC20PermitSignature(depositor1, teller2.address, usdc, ONE_ETHER.mul(3));
      ...
      expect(change_in_balances.vestingSolace).eq(bondInfo.payoutAmount);
      expect(change_in_balances.stakingSolace).eq(0);
      expect(change_in_balances.userUsdc).eq(ONE_ETHER.mul(-3));
      expect(change_in_balances.userDai).eq(0);
      expect(change_in_balances.daoUsdc).eq(ONE_ETHER.mul(3).mul(protocolFee).div(MAX_BPS));
      expect(change_in_balances.daoDai).eq(0);
      expect(change_in_balances.poolUsdc).eq(ONE_ETHER.mul(3).mul(MAX_BPS-protocolFee).div(MAX_BPS))
    ...
    it("test deposit 2 - capacityIsPayout = true && stake = true, deposit 3 USDC, starting SOLACE price of 2 USDC", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      MODIFIED_BOND_TERMS.capacityIsPayout = true;
    ...
    it("test deposit 3 - deposit 3 USDC, set startPrice = 1 but minimumPrice = 2", async function() {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.minimumPrice = ONE_ETHER.mul(2)
      await teller2.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      let protocolFee = PROTOCOL_FEE;
  ...
  describe("set terms", async function () {
    before("redeploy", async function () {
      ...
      await solace.connect(governor).addMinter(bondDepo.address);
      teller1 = (await deployContract(deployer, artifacts.BondTellerERC20)) as BondTellerErc20;
      await teller1.connect(governor).initialize("Solace DAI Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, dai.address, false, bondDepo.address);
      await bondDepo.connect(governor).addTeller(teller1.address);
      teller2 = await cloneTeller(teller1, "Solace USDC Bond", usdc.address, true);
    });
  ...
  describe("set addresses", function () {
    ...
    it("non governance cannot set addresses", async function () {
      await expect(teller1.connect(depositor1).setAddresses(solace.address, xslocker.address, underwritingPool.address, dao.address, dai.address, false, bondDepo2.address)).to.be.revertedWith("!governance");
    });
    it("validates input", async function () {
      await expect(teller1.connect(governor).setAddresses(ZERO_ADDRESS, xslocker.address, underwritingPool.address, dao.address, dai.address, false, bondDepo2.address)).to.be.revertedWith("zero address solace");
      await expect(teller1.connect(governor).setAddresses(solace.address, ZERO_ADDRESS, underwritingPool.address, dao.address, dai.address, false, bondDepo2.address)).to.be.revertedWith("zero address xslocker");
      await expect(teller1.connect(governor).setAddresses(solace.address, xslocker.address, ZERO_ADDRESS, dao.address, dai.address, false, bondDepo2.address)).to.be.revertedWith("zero address pool");
      await expect(teller1.connect(governor).setAddresses(solace.address, xslocker.address, underwritingPool.address, ZERO_ADDRESS, dai.address, false, bondDepo2.address)).to.be.revertedWith("zero address dao");
      ...
      await expect(teller1.connect(governor).setAddresses(solace.address, xslocker.address, underwritingPool.address, dao.address, dai.address, false, ZERO_ADDRESS)).to.be.revertedWith("zero address bond depo");
    })
    it("governance can set addresses", async function () {
      let tx = await teller1.connect(governor).setAddresses(solace2.address, xslocker2.address, underwritingPool2.address, dao2.address, usdc.address, true, bondDepo2.address);
      ...
      expect(await teller1.principal()).eq(usdc.address);
    ...
    it("uses new addresses", async function () {
      await usdc.connect(deployer).transfer(depositor1.address, ONE_ETHER.mul(100));
      await usdc.connect(depositor1).approve(teller1.address, constants.MaxUint256);
      ...
      expect(bal12.userUsdc).eq(ONE_ETHER.mul(-3));
      expect(bal12.daoDai).eq(0);
      expect(await usdc.balanceOf(dao2.address)).eq(ONE_ETHER.mul(3).mul(PROTOCOL_FEE).div(MAX_BPS));
      expect(bal12.poolDai).eq(0);
      expect(await usdc.balanceOf(underwritingPool2.address)).eq(ONE_ETHER.mul(3).mul(MAX_BPS-PROTOCOL_FEE).div(MAX_BPS));
  ...
  async function getBalances(teller: Contract, user: Wallet): Promise<Balances> {
    return {
      ...
      userUsdc: await usdc.balanceOf(user.address),
      daoDai: await dai.balanceOf(dao.address),
      daoUsdc: await usdc.balanceOf(dao.address),
      poolDai: await dai.balanceOf(underwritingPool.address),
      poolUsdc: await usdc.balanceOf(underwritingPool.address),
  ...
  function getBalancesDiff(balances1: Balances, balances2: Balances): Balances {
    return {
      ...
      daoUsdc: balances1.daoUsdc.sub(balances2.daoUsdc),
      poolDai: balances1.poolDai.sub(balances2.poolDai),
      poolUsdc: balances1.poolUsdc.sub(balances2.poolUsdc),
  ...
  let saltCounter = 0;
  async function cloneTeller(sourceTeller: BondTellerErc20, name: string, principal: string, isPermittable: boolean) {
    ++saltCounter;
    let nextSalt = toBytes32(saltCounter);
    let addr = await sourceTeller.calculateMinimalProxyDeploymentAddress(nextSalt);
    await sourceTeller.clone(name, governor.address, principal, isPermittable, nextSalt);
