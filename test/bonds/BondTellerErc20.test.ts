...
import { Solace, XSolacev1, MockErc20, MockErc20Permit, BondDepository, BondTellerErc20 } from "./../../typechain";
...
describe("BondTellerERC20", function() {
  ...
  describe("deposit", async function () {
    ...
    it("cannot deposit before start", async function () {
      ...
      await teller1.connect(governor).setTerms({startPrice: ONE_ETHER, minimumPrice: ONE_ETHER.mul(2), maxPayout: ONE_ETHER.mul(2), priceAdjNum: 1, priceAdjDenom: 10, capacity: ONE_ETHER.mul(10), capacityIsPayout: false, startTime: blockTimestamp+10, endTime: MAX_UINT40, vestingTerm: VESTING_TERM, halfLife: HALF_LIFE});
    ...
    it("cannot deposit after conclusion", async function () {
      const blockTimestamp = (await provider.getBlock('latest')).timestamp;
      await teller1.connect(governor).setTerms({startPrice: ONE_ETHER, minimumPrice: ONE_ETHER.mul(2), maxPayout: ONE_ETHER.mul(2), priceAdjNum: 1, priceAdjDenom: 10, capacity: ONE_ETHER.mul(10), capacityIsPayout: false, startTime: 0, endTime: blockTimestamp-1, vestingTerm: VESTING_TERM, halfLife: HALF_LIFE});
    ...
    it("cannot divide by zero price", async function () {
      await teller1.connect(governor).setTerms({startPrice: 1, minimumPrice: 0, maxPayout: ONE_ETHER.mul(2), priceAdjNum: 1, priceAdjDenom: 10, capacity: ONE_ETHER.mul(10), capacityIsPayout: false, startTime: 0, endTime: MAX_UINT40, vestingTerm: VESTING_TERM, halfLife: 1});
      const blockTimestamp = (await provider.getBlock('latest')).timestamp;
    ...
    it("cannot exceed capacity in principal", async function () {
      await teller1.connect(governor).setTerms({startPrice: ONE_ETHER.mul(2), minimumPrice: 0, maxPayout: ONE_ETHER.mul(2), priceAdjNum: 1, priceAdjDenom: 10, capacity: ONE_ETHER, capacityIsPayout: false, startTime: 0, endTime: MAX_UINT40, vestingTerm: VESTING_TERM, halfLife: HALF_LIFE});
    ...
    it("cannot exceed capacity in payout", async function () {
      await teller1.connect(governor).setTerms({startPrice: ONE_ETHER.mul(2), minimumPrice: 0, maxPayout: ONE_ETHER.mul(2), priceAdjNum: 1, priceAdjDenom: 10, capacity: ONE_ETHER, capacityIsPayout: true, startTime: 0, endTime: MAX_UINT40, vestingTerm: VESTING_TERM, halfLife: HALF_LIFE});
      expect(await teller1.capacity()).eq(ONE_ETHER);
    ...
    it("slippage protection", async function () {
      ...
      await expect(teller1.connect(depositor1).deposit(ONE_ETHER.mul(3), ONE_ETHER.mul(2), depositor1.address, false)).to.be.revertedWith("slippage protection");
      ...
      await expect(teller1.connect(depositor1).deposit(ONE_ETHER.mul(3), ONE_ETHER.mul(2), depositor1.address, true)).to.be.revertedWith("slippage protection");
    });
  ...
  describe("set addresses", function () {
    ...
    let solace2: Solace;
    let xsolace2: XSolacev1;
    let bondDepo2: BondDepository;

    before("redeploy", async function () {
      solace2 = (await deployContract(deployer, artifacts.SOLACE, [governor.address])) as Solace;
      xsolace2 = (await deployContract(deployer, artifacts.xSOLACEV1, [governor.address, solace.address])) as XSolacev1;
    ...
    it("non governance cannot set addresses", async function () {
      await expect(teller1.connect(depositor1).setAddresses(solace.address, xsolace.address, underwritingPool.address, dao.address, tkn1.address, bondDepo.address)).to.be.revertedWith("!governance");
    });
    it("validates input", async function () {
      await expect(teller1.connect(governor).setAddresses(ZERO_ADDRESS, xsolace.address, underwritingPool.address, dao.address, tkn1.address, bondDepo.address)).to.be.revertedWith("zero address solace");
      await expect(teller1.connect(governor).setAddresses(solace.address, ZERO_ADDRESS, underwritingPool.address, dao.address, tkn1.address, bondDepo.address)).to.be.revertedWith("zero address xsolace");
      await expect(teller1.connect(governor).setAddresses(solace.address, xsolace.address, ZERO_ADDRESS, dao.address, tkn1.address, bondDepo.address)).to.be.revertedWith("zero address pool");
      await expect(teller1.connect(governor).setAddresses(solace.address, xsolace.address, underwritingPool.address, ZERO_ADDRESS, tkn1.address, bondDepo.address)).to.be.revertedWith("zero address dao");
      ...
      await expect(teller1.connect(governor).setAddresses(solace.address, xsolace.address, underwritingPool.address, dao.address, tkn1.address, ZERO_ADDRESS)).to.be.revertedWith("zero address bond depo");
    })
    it("governance can set addresses", async function () {
      ...
      expect(await teller1.principal()).eq(tkn2.address);
      expect(await teller1.bondDepo()).eq(bondDepo2.address);
    });
    it("uses new addresses", async function () {
      await tkn2.connect(deployer).transfer(depositor1.address, ONE_ETHER.mul(100));
      await tkn2.connect(depositor1).approve(teller1.address, constants.MaxUint256);
      await teller1.connect(governor).setTerms({startPrice: ONE_ETHER, minimumPrice: ONE_ETHER.mul(2), maxPayout: ONE_ETHER.mul(2), priceAdjNum: 1, priceAdjDenom: 10, capacity: ONE_ETHER.mul(10), capacityIsPayout: false, startTime: 0, endTime: MAX_UINT40, vestingTerm: VESTING_TERM, halfLife: HALF_LIFE});
      let bal1 = await getBalances(teller1, depositor1);
      const blockTimestamp = (await provider.getBlock('latest')).timestamp;
      let predictedAmountOut = await teller1.calculateAmountOut(ONE_ETHER.mul(3), false);
      ...
      expect(bal12.userTkn2).eq(ONE_ETHER.mul(-3));
      expect(bal12.daoTkn1).eq(0);
      expect(await tkn2.balanceOf(dao2.address)).eq(ONE_ETHER.mul(3).mul(DAO_FEE).div(MAX_BPS));
      expect(bal12.poolTkn1).eq(0);
      expect(await tkn2.balanceOf(underwritingPool2.address)).eq(ONE_ETHER.mul(3).mul(MAX_BPS-DAO_FEE).div(MAX_BPS));
      expect(bal12.userBonds).eq(1);
      expect(bal12.totalBonds).eq(1);
      expect(bal12.tellerCapacity).eq(bondInfo.pricePaid.mul(-1));
      expectClose(bal12.tellerBondPrice, bondInfo.payoutAmount.div(10).mul(MAX_BPS).div(MAX_BPS-BOND_FEE), 1e14);
