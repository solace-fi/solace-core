...
import { Solace, XsLocker, Weth9, Weth10, BondDepository, BondTellerMatic } from "./../../typechain";
...
let teller1: BondTellerMatic;
let teller2: BondTellerMatic;
...
describe("BondTellerMATIC", function() {
  ...
  before(async function() {
    ...
    wmatic9 = (await deployContract(deployer, artifacts.WETH)) as Weth9;
    wmatic10 = (await deployContract(deployer, artifacts.WETH10)) as Weth10;
    await wmatic9.connect(deployer).deposit({value: ONE_ETHER.mul(100)});
  ...
  describe("before initialization", function () {
    it("can deploy implementation", async function () {
      teller1 = (await deployContract(deployer, artifacts.BondTellerMATIC)) as BondTellerMatic;
      await expectDeployed(teller1.address);
      await bondDepo.connect(governor).addTeller(teller1.address);
      teller2 = (await deployContract(deployer, artifacts.BondTellerMATIC)) as BondTellerMatic;
    ...
    it("reverts if zero governor", async function () {
      await expect(teller1.initialize("Solace MATIC Bond", ZERO_ADDRESS, solace.address, xslocker.address, underwritingPool.address, dao.address, wmatic9.address, false, bondDepo.address)).to.be.revertedWith("zero address governance");
    });
    it("reverts if zero solace", async function () {
      await expect(teller1.initialize("Solace MATIC Bond", governor.address, ZERO_ADDRESS, xslocker.address, underwritingPool.address, dao.address, wmatic9.address, false, bondDepo.address)).to.be.revertedWith("zero address solace");
    });
    it("reverts if zero xslocker", async function () {
      await expect(teller1.initialize("Solace MATIC Bond", governor.address, solace.address, ZERO_ADDRESS, underwritingPool.address, dao.address, wmatic9.address, false, bondDepo.address)).to.be.revertedWith("zero address xslocker");
    });
    it("reverts if zero pool", async function () {
      await expect(teller1.initialize("Solace MATIC Bond", governor.address, solace.address, xslocker.address, ZERO_ADDRESS, dao.address, wmatic9.address, false, bondDepo.address)).to.be.revertedWith("zero address pool");
    });
    it("reverts if zero dao", async function () {
      await expect(teller1.initialize("Solace MATIC Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, ZERO_ADDRESS, wmatic9.address, false, bondDepo.address)).to.be.revertedWith("zero address dao");
    });
    it("reverts if zero principal", async function () {
      await expect(teller1.initialize("Solace MATIC Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, ZERO_ADDRESS, false, bondDepo.address)).to.be.revertedWith("zero address principal");
    });
    it("reverts if zero bond depo", async function () {
      await expect(teller1.initialize("Solace MATIC Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, wmatic9.address, false, ZERO_ADDRESS)).to.be.revertedWith("zero address bond depo");
    });
  });

  describe("initialization", function () {
    it("inits", async function () {
      await teller1.initialize("Solace MATIC Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, wmatic9.address, false, bondDepo.address);
      await teller2.initialize("Solace MATIC Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, wmatic10.address, true, bondDepo.address);
    });
    it("inits with a name and symbol", async function () {
      expect(await teller1.name()).eq("Solace MATIC Bond");
    ...
    it("starts with correct principal", async function () {
      expect(await teller1.principal()).eq(wmatic9.address);
    });
  ...
  describe("pause/unpause", function () {
    ...
    it("cannot deposit while paused", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWmatic(1, 1, depositor1.address, false)).to.be.revertedWith("cannot deposit while paused");
    ...
    it("cannot depositSigned while paused", async function () {
      ...
      let { v, r, s } = await getERC20PermitSignature(depositor1, teller2.address, wmatic10, ONE_ETHER);
      await expect(teller2.connect(depositor1).depositWmaticSigned(ONE_ETHER, ONE_ETHER, depositor1.address, false, deadline, v, r, s)).to.be.revertedWith("cannot deposit while paused");
  ...
  describe("before setTerms() called", function() {
    ...
    it("cannot deposit() or depositSigned()", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWmatic(1, 1, depositor1.address, false)).to.be.revertedWith("not initialized");
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wmatic10, ONE_ETHER);
      await expect(teller2.connect(depositor2).depositWmaticSigned(ONE_ETHER, ONE_ETHER, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("not initialized");
    });
  });
  ...
  describe("ERC20 guards", function() {
    it("cannot deposit or depositSigned with insufficient balance", async function () {
      ...
      expect(await wmatic9.balanceOf(depositor1.address)).to.equal(0);
      await expect(teller1.connect(depositor1).depositWmatic(1, 1, depositor1.address, false)).to.be.revertedWith("ERC20: transfer amount exceeds balance");
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wmatic10, 1);
      await expect(teller2.connect(depositor2).depositWmaticSigned(1, 0, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("WETH: transfer amount exceeds balance");
    });
    it("cannot deposit without allowance", async function () {
      // Transfer 100 WMATIC9 to depositor1
      await wmatic9.connect(deployer).transfer(depositor1.address, 100);
      expect(await wmatic9.balanceOf(depositor1.address)).to.equal(100)
      await expect(teller1.connect(deployer).depositWmatic(100, 0, depositor1.address, false)).to.be.revertedWith("ERC20: transfer amount exceeds allowance");
      await wmatic9.connect(depositor1).transfer(deployer.address, 100);
    });
    it("cannot permit a non erc20permit token", async function () {
      let { v, r, s } = await getERC20PermitSignature(depositor1, teller1.address, wmatic9, 1, constants.MaxUint256, 1);
      await expect(teller1.connect(depositor1).depositWmaticSigned(1, 1, depositor1.address, false, deadline, v, r, s)).to.be.revertedWith("principal does not support permit");
    });
  })

  describe("term parameter guards", function() {
    before(async function() {
      // Transfer 100K WMATIC9 to depositor1
      await wmatic9.connect(depositor1).approve(teller1.address, constants.MaxUint256);
      await wmatic9.connect(deployer).transfer(depositor1.address, ONE_ETHER.mul(100));
      expect(await wmatic9.balanceOf(depositor1.address)).to.equal(ONE_ETHER.mul(100));

      // Transfer 100K WMATIC10 to depositor2
      await wmatic10.connect(deployer).transfer(depositor2.address, ONE_ETHER.mul(100));
      expect(await wmatic10.balanceOf(depositor2.address)).to.equal(ONE_ETHER.mul(100));
    })

    it("cannot deposit or depositSigned with a zero address depositor", async function() {
      await expect(teller1.connect(depositor1).depositWmatic(1, 1, ZERO_ADDRESS, false)).to.be.revertedWith("invalid address");
      ...
      // let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wmatic10, ONE_ETHER);
      // await expect(teller2.connect(depositor2).depositWmaticSigned(ONE_ETHER, ONE_ETHER, ZERO_ADDRESS, false, deadline, v, r, s)).to.be.revertedWith("invalid address");
    })
    it("cannot deposit or depositSigned before startTime", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWmatic(1, 1, depositor1.address, false)).to.be.revertedWith("bond not yet started");

      await teller2.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wmatic10, ONE_ETHER);
      await expect(teller2.connect(depositor2).depositWmaticSigned(ONE_ETHER, ONE_ETHER, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("bond not yet started");
    });
    it("cannot deposit or depositSigned after endTime", async function () {
      ...
      await teller1.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      await expect(teller1.connect(depositor1).depositWmatic(1, 1, depositor1.address, false)).to.be.revertedWith("bond concluded");

      await teller2.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wmatic10, ONE_ETHER);
      await expect(teller2.connect(depositor2).depositWmaticSigned(ONE_ETHER, ONE_ETHER, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("bond concluded");
    });
    it("cannot deposit or depositSigned if bondPrice decayed to 0", async function() {
      ...
      await expect(teller1.connect(depositor1).depositWmatic(1, 1, depositor1.address, false)).to.be.revertedWith("invalid price");
      ...
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wmatic10, ONE_ETHER);
      await expect(teller2.connect(depositor2).depositWmaticSigned(ONE_ETHER, ONE_ETHER, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("invalid price");
    })
    it("Given capacityIsPayout = false, deposit or depositSigned will revert if `principal paid > capacity`", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWmatic(ONE_ETHER.add(1), ONE_ETHER.mul(2), depositor1.address, false)).to.be.revertedWith("bond at capacity");
      ...
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wmatic10, ONE_ETHER.add(1));
      await expect(teller2.connect(depositor2).depositWmaticSigned(ONE_ETHER.add(1), ONE_ETHER, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("bond at capacity");
    });
    it("Given capacityIsPayout = true, deposit or depositSigned will revert if `payout > capacity`", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWmatic(ONE_ETHER.add(2), 0, depositor1.address, false)).to.be.revertedWith("bond at capacity");
      ...
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wmatic10, ONE_ETHER.add(2));
      await expect(teller2.connect(depositor2).depositWmaticSigned(ONE_ETHER.add(2), 0, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("bond at capacity");
    });
    it("calculateAmountIn, calculateAmountOut, deposit and depositSigned will respect maxPayout", async function() {
      ...
      await expect(teller1.connect(depositor1).depositWmatic(ONE_ETHER.add(2), 0, depositor1.address, false)).to.be.revertedWith("bond too large");
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wmatic10, ONE_ETHER.add(2));
      await expect(teller2.connect(depositor2).depositWmaticSigned(ONE_ETHER.add(2), 0, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("bond too large");
    })
    it("slippage protection - deposit() and depositSigned() respect minAmountOut", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWmatic(amountIn1.sub(1), ONE_ETHER, depositor1.address, false)).to.be.revertedWith("slippage protection");
      // Block-scope here to avoid v, r, s namespace conflict
      {
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wmatic10, amountIn1.sub(1));
      await expect(teller2.connect(depositor2).depositWmaticSigned(amountIn1.sub(1), ONE_ETHER, depositor2.address, false, deadline, v, r, s)).to.be.revertedWith("slippage protection");
      }

      const amountIn2 = await teller1.calculateAmountIn(ONE_ETHER, true);
      await expect(teller1.connect(depositor1).depositWmatic(amountIn2.sub(1), ONE_ETHER, depositor1.address, true)).to.be.revertedWith("slippage protection");
      {
      let { v, r, s } = await getERC20PermitSignature(depositor2, teller2.address, wmatic10, amountIn2.sub(1));
      await expect(teller2.connect(depositor2).depositWmaticSigned(amountIn2.sub(1), ONE_ETHER, depositor2.address, true, deadline, v, r, s)).to.be.revertedWith("slippage protection");
      }
    });
    it("cannot deposit if bondDepo is not solace minter", async function () {
      ...
      await expect(teller1.connect(depositor1).depositWmatic(2, 0, depositor1.address, false)).to.be.revertedWith("!minter");
      await solace.connect(governor).addMinter(bondDepo.address);
    });
    it("cannot deposit if teller not registered", async function () {
      await bondDepo.connect(governor).removeTeller(teller1.address);
      await expect(teller1.connect(depositor1).depositWmatic(2, 0, depositor1.address, false)).to.be.revertedWith("!teller");
      await bondDepo.connect(governor).addTeller(teller1.address);
    });
  })

  describe("depositMatic cases", function () {
    it("test deposit 1 - deposit 3 MATIC, starting SOLACE price of 2 MATIC", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      await teller1.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      ...
      // Tx to purchase bond
      let tx1 = await teller1.connect(depositor1).depositMatic(ONE_ETHER, depositor1.address, false, {value: ONE_ETHER.mul(3)});
      ...
      expect(change_in_balances.userMatic).eq(ONE_ETHER.mul(-3).sub(gasCost));
      expect(change_in_balances.userWmatic9).eq(0);
      expect(change_in_balances.userWmatic10).eq(0);
      expect(change_in_balances.daoMatic).eq(ONE_ETHER.mul(3).mul(protocolFee).div(MAX_BPS));
      expect(change_in_balances.daoWmatic9).eq(0);
      expect(change_in_balances.daoWmatic10).eq(0);
      expect(change_in_balances.poolMatic).eq(ONE_ETHER.mul(3).mul(MAX_BPS-protocolFee).div(MAX_BPS))
      expect(change_in_balances.poolWmatic9).eq(0);
      expect(change_in_balances.poolWmatic10).eq(0);
    ...
    it("test deposit 2 - capacityIsPayout = true && stake = true, deposit 3 MATIC, starting SOLACE price of 2 MATIC", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      MODIFIED_BOND_TERMS.capacityIsPayout = true;
      ...
      // Deposit
      let tx1 = await teller1.connect(depositor1).depositMatic(ONE_ETHER, depositor1.address, true, {value: ONE_ETHER.mul(3)});
      let receipt = await tx1.wait();
      let gasCost = receipt.gasUsed.mul(receipt.effectiveGasPrice);
    ...
    it("test deposit 3 - deposit 3 MATIC, set startPrice = 1 but minimumPrice = 2", async function() {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
  ...
  describe("claimPayout after depositMatic cases", function() {
    ...
    it("t = 0, expect claimPayout will work but there will be miniscule payout", async function() {
      ...
      expect(change_in_balances.userWmatic9).to.eq(0)
      expect(change_in_balances.userWmatic10).to.eq(0)
      expect(change_in_balances.daoWmatic9).to.eq(0)
      expect(change_in_balances.daoWmatic10).to.eq(0)
      expect(change_in_balances.poolWmatic9).to.eq(0)
      expect(change_in_balances.poolWmatic10).to.eq(0)
      ...
      expect(change_in_balances_depositor2.userWmatic9).to.eq(0)
      expect(change_in_balances_depositor2.userWmatic10).to.eq(0)
  ...
  describe("depositWmatic cases", function () {
    before("redeploy", async function () {
      ...
      await solace.connect(governor).addMinter(bondDepo.address);
      teller1 = (await deployContract(deployer, artifacts.BondTellerMATIC)) as BondTellerMatic;
      await teller1.connect(governor).initialize("Solace WMATIC9 Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, wmatic9.address, false, bondDepo.address);
      await bondDepo.connect(governor).addTeller(teller1.address);
      teller2 = (await deployContract(deployer, artifacts.BondTellerMATIC)) as BondTellerMatic;
      await teller2.connect(governor).initialize("Solace WMATIC10 Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, wmatic10.address, true, bondDepo.address);
      await bondDepo.connect(governor).addTeller(teller2.address);
      await wmatic9.connect(depositor1).approve(teller1.address, constants.MaxUint256);
    });
    it("test deposit 1 - deposit 3 WMATIC9, starting SOLACE price of 2 WMATIC9", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      await teller1.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      ...
      // Tx to purchase bond
      let tx1 = await teller1.connect(depositor1).depositWmatic(ONE_ETHER.mul(3), ONE_ETHER, depositor1.address, false);
      let receipt = await tx1.wait();
      let gasCost = receipt.gasUsed.mul(receipt.effectiveGasPrice);
      ...
      expect(change_in_balances.vestingSolace).eq(bondInfo.payoutAmount);
      expect(change_in_balances.stakingSolace).eq(0);
      expect(change_in_balances.userMatic).eq(gasCost.mul(-1));
      expect(change_in_balances.userWmatic9).eq(ONE_ETHER.mul(-3));
      expect(change_in_balances.userWmatic10).eq(0);
      expect(change_in_balances.daoMatic).eq(0);
      expect(change_in_balances.daoWmatic9).eq(ONE_ETHER.mul(3).mul(protocolFee).div(MAX_BPS));
      expect(change_in_balances.daoWmatic10).eq(0);
      expect(change_in_balances.poolMatic).eq(0);
      expect(change_in_balances.poolWmatic9).eq(ONE_ETHER.mul(3).mul(MAX_BPS-protocolFee).div(MAX_BPS))
      expect(change_in_balances.poolWmatic10).eq(0);
      expect(change_in_balances.userBonds).eq(1);
      expect(change_in_balances.totalBonds).eq(1);
      expect(change_in_balances.userLocks).eq(0);
    ...
    it("test deposit 2 - capacityIsPayout = true && stake = true, deposit 3 WMATIC9, starting SOLACE price of 2 WMATIC9", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      MODIFIED_BOND_TERMS.capacityIsPayout = true;
      ...
      // Deposit
      let tx1 = await teller1.connect(depositor1).depositWmatic(ONE_ETHER.mul(3), ONE_ETHER, depositor1.address, true);
      let receipt = await tx1.wait();
      let gasCost = receipt.gasUsed.mul(receipt.effectiveGasPrice);
    ...
    it("test deposit 3 - deposit 3 WMATIC9, set startPrice = 1 but minimumPrice = 2", async function() {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.minimumPrice = ONE_ETHER.mul(2)
      await teller1.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      let protocolFee = PROTOCOL_FEE;
  ...
  describe("claimPayout after depositWmatic cases", function() {
    it("cannot claimPayout for a non-existent bondID", async function () {
      await expect(teller1.connect(depositor1).claimPayout(999)).to.be.revertedWith("query for nonexistent token");
    });
    it("cannot claimPayout for a bondID that you are not that owner of, or are approved to transfer", async function () {
  ...
  describe("depositWmaticSigned cases", function () {
    before("redeploy", async function () {
      solace = (await deployContract(deployer, artifacts.SOLACE, [governor.address])) as Solace;
      xslocker = (await deployContract(deployer, artifacts.xsLocker, [governor.address, solace.address])) as XsLocker;
      bondDepo = (await deployContract(deployer, artifacts.BondDepository, [governor.address, solace.address])) as BondDepository;
      ...
      await teller2.connect(governor).initialize("Solace WMATIC10 Bond", governor.address, solace.address, xslocker.address, underwritingPool.address, dao.address, wmatic10.address, true, bondDepo.address);
      await bondDepo.connect(governor).addTeller(teller2.address);
      await wmatic9.connect(depositor1).approve(teller2.address, constants.MaxUint256);
      await wmatic10.connect(depositor1).approve(teller2.address, constants.MaxUint256);
      await wmatic10.connect(depositor1).deposit({value: ONE_ETHER.mul(100)});
    });
    it("test deposit 1 - deposit 3 WMATIC10, starting SOLACE price of 2 WMATIC10", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      await teller2.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      ...
      // Tx to purchase bond
      let { v, r, s } = await getERC20PermitSignature(depositor1, teller2.address, wmatic10, ONE_ETHER.mul(3));
      let tx1 = await teller2.connect(depositor1).depositWmaticSigned(ONE_ETHER.mul(3), ONE_ETHER, depositor1.address, false, deadline, v, r, s);
      let receipt = await tx1.wait();
      let gasCost = receipt.gasUsed.mul(receipt.effectiveGasPrice);
      ...
      expect(change_in_balances.userMatic).eq(gasCost.mul(-1));
      expect(change_in_balances.userWmatic9).eq(0);
      expect(change_in_balances.userWmatic10).eq(ONE_ETHER.mul(-3));
      expect(change_in_balances.daoMatic).eq(0);
      expect(change_in_balances.daoWmatic9).eq(0);
      expect(change_in_balances.daoWmatic10).eq(ONE_ETHER.mul(3).mul(protocolFee).div(MAX_BPS));
      expect(change_in_balances.poolMatic).eq(0);
      expect(change_in_balances.poolWmatic9).eq(0);
      expect(change_in_balances.poolWmatic10).eq(ONE_ETHER.mul(3).mul(MAX_BPS-protocolFee).div(MAX_BPS))
      expect(change_in_balances.userBonds).eq(1);
      expect(change_in_balances.totalBonds).eq(1);
      expect(change_in_balances.userLocks).eq(0);
      expect(change_in_balances.totalLocks).eq(0);
    ...
    it("test deposit 2 - capacityIsPayout = true && stake = true, deposit 3 WMATIC10, starting SOLACE price of 2 WMATIC10", async function () {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.startPrice = ONE_ETHER.mul(2)
      MODIFIED_BOND_TERMS.minimumPrice = 0
      MODIFIED_BOND_TERMS.capacityIsPayout = true;
      ...
      // Deposit
      let { v, r, s } = await getERC20PermitSignature(depositor1, teller2.address, wmatic10, ONE_ETHER.mul(3));
      let tx1 = await teller2.connect(depositor1).depositWmaticSigned(ONE_ETHER.mul(3), ONE_ETHER, depositor1.address, true, deadline, v, r, s);
      let receipt = await tx1.wait();
      let gasCost = receipt.gasUsed.mul(receipt.effectiveGasPrice);
    ...
    it("test deposit 3 - deposit 3 WMATIC10, set startPrice = 1 but minimumPrice = 2", async function() {
      let MODIFIED_BOND_TERMS = {...DEFAULT_BOND_TERMS}
      MODIFIED_BOND_TERMS.minimumPrice = ONE_ETHER.mul(2)
      await teller2.connect(governor).setTerms(MODIFIED_BOND_TERMS);
      let protocolFee = PROTOCOL_FEE;
  ...
  describe("claimPayout after depositWmaticSigned cases", function() {
    it("cannot claimPayout for a non-existent bondID", async function () {
  ...
  describe("set addresses", function () {
    ...
    it("non governance cannot set addresses", async function () {
      await expect(teller1.connect(depositor1).setAddresses(solace.address, xslocker.address, underwritingPool.address, dao.address, wmatic9.address, false, bondDepo2.address)).to.be.revertedWith("!governance");
    });
    it("validates input", async function () {
      await expect(teller1.connect(governor).setAddresses(ZERO_ADDRESS, xslocker.address, underwritingPool.address, dao.address, wmatic9.address, false, bondDepo2.address)).to.be.revertedWith("zero address solace");
      await expect(teller1.connect(governor).setAddresses(solace.address, ZERO_ADDRESS, underwritingPool.address, dao.address, wmatic9.address, false, bondDepo2.address)).to.be.revertedWith("zero address xslocker");
      await expect(teller1.connect(governor).setAddresses(solace.address, xslocker.address, ZERO_ADDRESS, dao.address, wmatic9.address, false, bondDepo2.address)).to.be.revertedWith("zero address pool");
      await expect(teller1.connect(governor).setAddresses(solace.address, xslocker.address, underwritingPool.address, ZERO_ADDRESS, wmatic9.address, false, bondDepo2.address)).to.be.revertedWith("zero address dao");
      ...
      await expect(teller1.connect(governor).setAddresses(solace.address, xslocker.address, underwritingPool.address, dao.address, wmatic9.address, false, ZERO_ADDRESS)).to.be.revertedWith("zero address bond depo");
    })
    it("governance can set addresses", async function () {
      let tx = await teller1.connect(governor).setAddresses(solace2.address, xslocker2.address, underwritingPool2.address, dao2.address, wmatic10.address, true, bondDepo2.address);
      ...
      expect(await teller1.principal()).eq(wmatic10.address);
    ...
    it("uses new addresses", async function () {
      await wmatic10.connect(depositor1).approve(teller1.address, constants.MaxUint256);
      ...
      expect(bal12.userWmatic9).eq(0);
      expect(bal12.userWmatic10).eq(ONE_ETHER.mul(-3));
      expect(bal12.daoWmatic9).eq(0);
      expect(await wmatic10.balanceOf(dao2.address)).eq(ONE_ETHER.mul(3).mul(PROTOCOL_FEE).div(MAX_BPS));
      expect(bal12.poolWmatic9).eq(0);
      expect(await wmatic10.balanceOf(underwritingPool2.address)).eq(ONE_ETHER.mul(3).mul(MAX_BPS-PROTOCOL_FEE).div(MAX_BPS));
  ...
  async function getBalances(teller: Contract, user: Wallet): Promise<Balances> {
    return {
      ...
      userMatic: await provider.getBalance(user.address),
      userWmatic9: await wmatic9.balanceOf(user.address),
      userWmatic10: await wmatic10.balanceOf(user.address),
      daoMatic: await provider.getBalance(dao.address),
      daoWmatic9: await wmatic9.balanceOf(dao.address),
      daoWmatic10: await wmatic10.balanceOf(dao.address),
      poolMatic: await provider.getBalance(underwritingPool.address),
      poolWmatic9: await wmatic9.balanceOf(underwritingPool.address),
      poolWmatic10: await wmatic10.balanceOf(underwritingPool.address),
  ...
  function getBalancesDiff(balances1: Balances, balances2: Balances): Balances {
    return {
      ...
      userMatic: balances1.userMatic.sub(balances2.userMatic),
      userWmatic9: balances1.userWmatic9.sub(balances2.userWmatic9),
      userWmatic10: balances1.userWmatic10.sub(balances2.userWmatic10),
      daoMatic: balances1.daoMatic.sub(balances2.daoMatic),
      daoWmatic9: balances1.daoWmatic9.sub(balances2.daoWmatic9),
      daoWmatic10: balances1.daoWmatic10.sub(balances2.daoWmatic10),
      poolMatic: balances1.poolMatic.sub(balances2.poolMatic),
      poolWmatic9: balances1.poolWmatic9.sub(balances2.poolWmatic9),
      poolWmatic10: balances1.poolWmatic10.sub(balances2.poolWmatic10),
